...
//
//  Title	  : NIIMAQdx.h
//  Created	  : 1453772544 seconds after 1/1/1970 12:00:00 UTC 
...
typedef enum IMAQdxError_enum {
    ...
    IMAQdxErrorTestPacketNotReceived,           // The system cannot acquire images. The packet size may be too large for the network setup or a firewall may be blocking the network traffic from the camera. For more information, visit ni.com/info and enter 'GigEVisionNetworkConnectivity'
    ...
    IMAQdxErrorRingAcqImageSizeNotAligned,      // For a ring acquisition with both image and chunk data, the total image size in bytes must be a multiple of the size alignment specified by the Required Payload Size Alignment attribute.
    IMAQdxErrorRingAcqChunkDataSizeTooBig,      // For a ring acquisition, the chunk data size must be less than the maximum payload transfer size specified by the Actual Maximum Payload Transfer Size attribute.
    IMAQdxErrorBufferNotExtracted,              // The requested buffer to release is not currently extracted.
    IMAQdxErrorDuplicateImageInRing,            // Duplicate images were found in the ring acquisition configuration.
    IMAQdxErrorImageNotConfiguredInRing,        // The image passed is not currently configured in the ring.
    IMAQdxErrorMismatchedBorders,               // The ring acquisition is configured to use images with different border sizes.  All images in a ring acquisition must have the same border size.
    IMAQdxErrorRingAcqDecodeRequired,           // For ring acquisitions, manipulating the image data prior to extraction is not allowed. The combination of the Pixel Format, Output Image Type, Pixel Signedness, Swap Pixel Bytes, and Shift Pixel Bits attributes requires the image data to be altered.
    IMAQdxErrorRingAcqNonZeroBorder,            // For a ring acquisition with this camera, the images must have a border size of 0 pixels.
    IMAQdxErrorRingAcqWidthNotAligned,          // For a ring acquisition with this camera, the image width must be a multiple of the image's required line alignment.
    IMAQdxErrorRingAcqOnly,                     // This function can only be called within the context of a ring acquisition.
    IMAQdxErrorRingAcqNotEnoughImages,          // The ring acquisition was not configured with enough images to meet the minimum requirements for a continuous acquisition.
    IMAQdxErrorImageDisposed,                   // The requested image has been disposed and is no longer valid.
    IMAQdxErrorRingAcqPaddingCapabilityExceeded,// The padding necessary to transfer the configured image using a ring acquisition is more than the camera is capable of providing. Reduce the amount of padding required by reducing the image border size or changing the image width.
    IMAQdxErrorNIGEVUpdateRequired,             // The NI-GEV driver is too old to support this functionality. Please update it to a newer version.
    IMAQdxErrorPacketSizeOver9000,              // The desired packet size is larger than the maximum value of 9000 supported by the network interface.
    IMAQdxErrorAttributeSetFailure,             // There was a failure when trying to set the value of a writeable camera attribute.
...
typedef enum IMAQdxBufferNumberMode_enum {
    ...
    IMAQdxBufferNumberModeEvery,
    IMAQdxBufferNumberModeLastNew,
...
//==============================================================================
//  Timeout parameter special value constants
//==============================================================================
#define IMAQDX_TIMEOUT_INFINITE             -1
#define IMAQDX_TIMEOUT_USE_DRIVER_SETTING   -2
...
IMAQdxError NI_FUNC IMAQdxConfigureRingAcquisition(IMAQdxSession id, bool32 continuous, Image* images[], uInt32 count);
IMAQdxError NI_FUNC IMAQdxExtractImage(IMAQdxSession id, IMAQdxBufferNumberMode mode, uInt32 desiredBufferNumber, uInt32* actualBufferNumber, Int32 timeout, bool32 releasePrevious, Image** image);
IMAQdxError NI_FUNC IMAQdxReleaseImage(IMAQdxSession id, Image* image, uInt32 bufferNumber);
IMAQdxError NI_FUNC IMAQdxGetImage2(IMAQdxSession id, Image* image, IMAQdxBufferNumberMode mode, uInt32 desiredBufferNumber, uInt32* actualBufferNumber, Int32 timeoutMs);
IMAQdxError NI_FUNC IMAQdxWriteRegister2(IMAQdxSession id, uInt64 offset, uInt32 value);
IMAQdxError NI_FUNC IMAQdxReadRegister2(IMAQdxSession id, uInt64 offset, uInt32* value);
IMAQdxError NI_FUNC IMAQdxWriteMemory2(IMAQdxSession id, uInt64 offset, const char* values, uInt32 count);
IMAQdxError NI_FUNC IMAQdxReadMemory2(IMAQdxSession id, uInt64 offset, char* values, uInt32 count);
